...
unsigned long long int fib_wrapper_recursive(
    unsigned long long int number, unsigned long long int* memo);
unsigned long long int fib_wrapper_iterative(
    unsigned long long int number, unsigned long long int* memo);
    ...
    unsigned long long int number, unsigned long long int* memo)
{
    ...
    memo[number] = fib_wrapper_recursive(number - 1, memo)
                   + fib_wrapper_recursive(number - 2, memo);
    return memo[number];
}
...
unsigned long long int fibonacci_recursive(unsigned long long int number)
{ // new memoization functions
    unsigned long long int* memo
        = calloc(number + 1, sizeof(unsigned long long int));
